...
- **API Key 보호**: Edge Functions의 환경변수에 저장
- **CORS**: 허용된 도메인만 접근 가능

### 9.2 성능
- **토큰 재사용**: 불필요한 OAuth 호출 방지
- **Haversine 계산 최적화**: 클라이언트 사이드 병렬 처리
- **지도 마커 최적화**: 마커 재사용, 클러스터링
- **인덱스**: `kakao_id`, `user_kakao_id`, `meetup_id`에 DB 인덱스 적용

### 9.3 확장성
- **Supabase Edge Functions**: 서버리스 아키텍처
- **PostgreSQL**: 수평 확장 가능
- **거리 계산**: Haversine 공식으로 빠르고 정확한 직선거리 계산
...
# Kakao (Supabase Secrets)
KAKAO_CLIENT_ID=xxx
KAKAO_CLIENT_SECRET=xxx
KAKAO_JAVASCRIPT_KEY=xxx

# Kakao Maps (Frontend)
KAKAO_MAPS_APP_KEY=xxx
```

### 10.2 배포 플랫폼
- **Frontend**: Figma Make / Vercel / Netlify
- **Backend**: Supabase (자동 배포)
